...
export default function SignUpPage() {
  ...
  const signInGoogle = async () => {
    ...
    setLoading(true);
    try {
      // важно: /auth/callback должен существовать
      ...
      // дальше будет редирект на Google, setLoading(false) не нужен
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-white">
      <div className="mx-auto flex max-w-[1200px] justify-center px-6 pt-[110px]">
        <div className="w-full max-w-[520px] text-center">
          ...
          >
            {mode === "email" ? "Зарегистрироваться" : "Подтвердить код"}
          </button>
          ...
          <div className="mt-6 text-[13px] text-black/70">
            Уже есть аккаунт?{" "}
            <a href="/login" className="font-medium text-[#6B5CFF] hover:underline">
              Войти
            </a>
          </div>
